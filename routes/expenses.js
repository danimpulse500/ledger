...
router.get('/', (req, res) => {
  ...
  const categories = db.prepare('SELECT DISTINCT category FROM expenses WHERE org_id = ? ORDER BY category').all(orgId).map((r) => r.category);
...
router.post('/', (req, res) => {
  ...
  db.prepare(`INSERT INTO expenses (org_id, description, category, amount, expense_date, vendor, created_by) VALUES (?,?,?,?,?,?,?)`)
...
router.get('/:id/edit', (req, res) => {
  const orgId = req.session.user.org_id;
  const expense = db.prepare('SELECT * FROM expenses WHERE id = ? AND org_id = ?').get(req.params.id, orgId);
...
router.put('/:id', (req, res) => {
  ...
  const result = db.prepare(`UPDATE expenses SET description=?, category=?, amount=?, expense_date=?, vendor=? WHERE id=? AND org_id=?`)
...
router.delete('/:id', (req, res) => {
  const orgId = req.session.user.org_id;
  db.prepare('DELETE FROM expenses WHERE id = ? AND org_id = ?').run(req.params.id, orgId);
  res.redirect('/expenses');
});
