...
router.post('/login', async (req, res, next) => {
  try {
    ...
    const admin = await db.prepare('SELECT * FROM platform_admins WHERE email = ?').get((email || '').toLowerCase().trim());
...
router.post('/organizations', async (req, res, next) => {
  ...
  if (!name || !slug || !admin_name || !admin_email || !admin_password || admin_password.length < 8) {
    const organizations = (await db.prepare('SELECT o.*, (SELECT COUNT(*) FROM users u WHERE u.org_id = o.id) AS member_count FROM organizations o ORDER BY o.created_at DESC').all()) || [];
  ...
  try {
    await db.exec('BEGIN;');
    const orgInfo = await db.prepare(`INSERT INTO organizations (name, slug, status, plan) VALUES (?, ?, 'active', ?)`).run(name.trim(), cleanSlug, plan || 'starter');
    const orgId = orgInfo.lastInsertRowid;

    // Create default company settings for new organization
    await db.prepare(`INSERT INTO company_settings (org_id, company_name) VALUES (?, ?)`).run(orgId, name.trim());

    // Create initial admin user for new organization
    const hash = bcrypt.hashSync(admin_password, 10);
    await db.prepare(`INSERT INTO users (org_id, name, email, password_hash, role) VALUES (?, ?, ?, ?, 'admin')`)
...
router.get('/organizations/:id', async (req, res, next) => {
  try {
    ...
    const members = (await db.prepare('SELECT id, name, email, role, created_at FROM users WHERE org_id = ? ORDER BY created_at ASC').all(org.id)) || [];
    const settings = await db.prepare('SELECT * FROM company_settings WHERE org_id = ?').get(org.id);
...
router.post('/organizations/:id/status', async (req, res, next) => {
  try {
    ...
    await db.prepare('UPDATE organizations SET status = ? WHERE id = ?').run(status, req.params.id);
...
router.post('/organizations/:id/plan', async (req, res, next) => {
  try {
    const { plan } = req.body;
    ...
    await db.prepare('UPDATE organizations SET plan = ? WHERE id = ?').run(plan, req.params.id);
    res.redirect(`/platform/organizations/${req.params.id}?updated=1`);
  } catch (err) {
    next(err);
  }
